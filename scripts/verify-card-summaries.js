...
function main() {
    ...
    for (const [file, meta] of entries) {
        ...
        for (const c of claims) {
            checkedClaims++;
            const hit = c.variants.some(v => hay.includes(normalize(String(v)).toLowerCase()))
                     || (c.variantRe && c.variantRe.test(hay));
            if (hit) {
                if (VERBOSE) console.log(`     ok  ${file}  ${c.kind} ${c.token}`);
                continue;
            }
            console.log(`  ❌ ${file}: card summary ${c.kind} "${c.token}" does not appear in the article it links to`);
            problems++;
        }
    }
