...
**Rendering** lives in `billTypeBadge()` / `BILL_TYPE_LABELS` / `BILL_TYPE_TIPS` (`app-render.js`); styling is `.status-type` (`styles-bills.css`). The badge appears wherever `renderBill()` runs — index cards, the bills page, and the full bill page (which reuses `renderBill()` via `bill.js`).
