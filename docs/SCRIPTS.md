...
| `node scripts/generate_reps.js` | Update rep profiles + rebuild index. Also re-runs the **second quote linker** (`lib/quote-link.js`) over `data/quotes.json`: links an unlinked floor quote to a cached bill ONLY on an explicit citation (sole cached bill named by number in the stored debate heading, else in the quote text). Never keyword-scores — a wrong link is worse than no link, and a keyword scorer re-created human-reverted mislinks on every run. Teeth: `npm run quotes:link:test` |
...
| `node scripts/verify-card-summaries.js [--verbose]` (`npm run verify-cards`) | **Card-summary staleness guard**: every vote tally, dollar amount, percentage, date, and number in an `articles/index.html` card summary (`data/articles-index.json`) must still appear in the article that card links to. Bare numbers are anchored to the noun they modify in the summary (a 212-bill roster contains every small integer, so presence alone proves nothing) and month-precision dates are satisfied by any day in that month. Runs inside `preflight.js` |
| `node scripts/test-quote-link.js` (`npm run quotes:link:test`) | **Teeth for the second quote linker.** Carries the retired keyword scorer as a frozen fixture, runs both linkers over the real stored corpus (read-only), and asserts the old one still reproduces the 2026-08-27 Grassley→RESULTS-Act mislink while the hardened rule refuses it; plus unit cases for every accept/reject branch. Runs inside `npm test`. Corpus checks SKIP (never fail) if the fixture quote is absent or deliberately linked |
| `node scripts/preflight.js [--verbose]` | Structural checks across every rendered HTML page: JSON-LD, smart quotes, entity @ids, bylines, theme, internal links, trust links, canonical/title, **title uniqueness, meta descriptions, sitemap↔disk both directions, district-geography sourcing (qa-geo-verify), card-summary staleness (verify-card-summaries)** |
